...
.vcard{background:var(--card);border:1px solid var(--border);border-radius:14px;padding:20px;margin-bottom:14px}.vtitle{font-size:16px;font-weight:700;color:var(--cyan);margin-bottom:7px}.hint{color:var(--muted);font-size:12.5px;margin-bottom:15px}.form-group{margin-bottom:15px}label{display:block;color:var(--muted);font-size:12.5px;margin-bottom:6px;font-weight:600}input,textarea,select{width:100%;background:var(--input);border:1px solid var(--border);border-radius:12px;color:var(--text);font:inherit;padding:11px 13px;min-height:44px}textarea{min-height:120px;resize:vertical}input:focus,textarea:focus,select:focus{outline:none;border-color:var(--cyan);box-shadow:0 0 0 2px rgba(0,229,255,.1)}input[type=file]{padding:9px}.btn-row{display:flex;gap:9px;flex-wrap:wrap;margin-top:14px}.btn{display:inline-flex;align-items:center;justify-content:center;gap:7px;min-height:44px;padding:10px 17px;border-radius:9px;border:1px solid transparent;font:600 13px inherit;cursor:pointer}.btn:disabled{opacity:.5;cursor:not-allowed}.primary{background:linear-gradient(135deg,var(--cyan),#00b8d4);color:#07101b}.secondary{background:var(--card2);border-color:var(--strong);color:var(--cyan)}.purple{background:linear-gradient(135deg,var(--purple),#9b7dff);color:#fff}.ghost{background:transparent;border-color:var(--border);color:var(--muted)}.success{background:rgba(0,230,118,.12);border-color:rgba(0,230,118,.3);color:var(--success)}.danger{background:rgba(255,82,82,.1);border-color:rgba(255,82,82,.3);color:#ff8a8a}
.choice-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.choice-card{background:var(--card2);border:1px solid var(--border);border-radius:13px;padding:16px;text-align:left;color:var(--text);cursor:pointer}.choice-card:hover{border-color:var(--cyan)}.choice-card strong{display:block;color:var(--cyan);font-size:15px;margin-bottom:4px}.choice-card span{color:var(--muted);font-size:12px}
.process{text-align:center;padding:30px 15px}.process-icon{font-size:34px;margin-bottom:7px}.process h2{font-size:18px;color:var(--cyan);margin-bottom:16px}.status-list{max-width:470px;margin:auto;text-align:left;background:var(--input);border:1px solid var(--border);border-radius:12px;padding:14px}.status-line{padding:6px 0;color:var(--muted)}.status-line.current{color:var(--cyan);font-weight:700}.status-line.done{color:var(--success)}
.audio-box{background:var(--input);border:1px solid var(--border);border-radius:12px;padding:16px}.audio-box audio{width:100%}.result-text{background:var(--input);border:1px solid var(--border);border-radius:10px;padding:13px;white-space:pre-wrap;min-height:120px}.srt-box{font-family:'Courier New',monospace;font-size:12.5px;line-height:1.6;min-height:230px}.result-label{font-weight:700;color:var(--cyan);margin:12px 0 7px}.pro-note{color:var(--warn);font-size:12px;margin-top:10px}.error-box{background:rgba(255,82,82,.08);border:1px solid rgba(255,82,82,.3);color:#ffb0b0;border-radius:10px;padding:12px;margin-top:12px}.top-actions{display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-bottom:12px}.source-note{font-size:11.5px;color:var(--muted);background:rgba(123,92,255,.08);border:1px solid rgba(123,92,255,.2);border-radius:8px;padding:8px 10px;margin-bottom:12px}
...
@media(max-width:700px){.mode-grid,.choice-grid{grid-template-columns:1fr}.mode-card{min-height:155px}.vcard{padding:15px}.voice-stepper{justify-content:flex-start}.vlink{width:18px;flex-basis:18px}.btn{flex:0 1 auto}.top-actions .btn{width:100%}}
@media(min-width:701px) and (max-width:1000px){.mode-grid{grid-template-columns:1fr;max-width:620px}}
</style>
</head>
<body>
...
var VOICE_STATE={
  ...
  processingState:null, errorState:null, source:''
};
...
function showLoading(text){document.getElementById('voiceLoadingText').textContent=text;document.getElementById('voiceLoading').classList.add('show');}
function hideLoading(){document.getElementById('voiceLoading').classList.remove('show');}
function saveDraft(){
...
function restoreDraft(){
  try{
    var raw=localStorage.getItem(voiceDraftKey);if(!raw)return;
    var d=JSON.parse(raw),s=d.state||{};
    if(d.tts)document.getElementById('ttsText').value=d.tts;
    if(d.speaking)document.getElementById('speakingStyle').value=d.speaking;
    if(d.voice)document.getElementById('voiceName').value=d.voice;
    if(d.instruction)document.getElementById('voiceInstruction').value=d.instruction;
    if(d.audience)document.getElementById('audience').value=d.audience;
    VOICE_STATE.translationDirection=d.dir||'MY_TO_CN';
    if(s.voiceMode&&d.tts){VOICE_STATE.voiceMode=s.voiceMode;}
    if(d.srt&&document.getElementById('srtEditor'))document.getElementById('srtEditor').value=d.srt;
    if(d.transcript&&document.getElementById('textResult'))document.getElementById('textResult').value=d.transcript;
...
function renderStepper(items,current){
  ...
  items.forEach(function(x,i){
    ...
    h+='<div class="vstep '+cls+'">'+(x.n<current?'✓ ':'')+esc(x.label)+'</div>';
...
function goHome(clear){
  if(clear){VOICE_STATE={voiceMode:null,voiceStep:0,voiceInput:null,voiceResult:null,audioResult:null,srtResult:null,translationDirection:'MY_TO_CN',translationResult:null,processingState:null,errorState:null,source:''};LAST_AUDIO={base64:'',mime:'audio/wav',url:''};MEDIA_AUDIO={base64:'',mime:'',fileName:''};translatedSrt='';}
  setScreen('home');document.getElementById('voiceStepper').innerHTML='';document.getElementById('voiceWorkflowBody').innerHTML='';
}
function voiceStartMode(mode){
  VOICE_STATE.voiceMode=mode;VOICE_STATE.voiceStep=1;VOICE_STATE.errorState=null;
  setScreen('workflow');
...
function renderTextInput(){
  ...
  if(VOICE_STATE.source){
    // transfer content was already placed by applyContentTransfer()
  }
}
...
function submitTextToVoice(){
  ...
  }).catch(function(e){renderVoiceError('tts',e);});
}
...
function renderVoiceResult(){
  VOICE_STATE.voiceStep=3;renderStepper(VOICE_STEPPER,3);
...
function startSrtFromVoice(){
  ...
  VOICE_STATE.processingState='srt';renderSrtInput('voice');
}
function renderSrtInput(source){
  VOICE_STATE.voiceStep=1;renderStepper(SRT_STEPPER,1);
...
function submitSrt(source){
  ...
  }).catch(function(e){renderGenericError('srt',source,e);});
}
...
function submitTranslation(source){
  ...
  }).catch(function(e){renderGenericError('translation',source,e);});
}
...
function renderTranslationResult(source){
  VOICE_STATE.voiceStep=3;renderStepper(TRANSLATE_STEPPER,3);
...
function renderGenericError(kind,source,e){
  var key=kind==='translation'?'translate_error':kind==='srt'?'srt_error':'request_error';
  var msg=friendlyError({error:key},'ဆောင်ရွက်ရာတွင် အခက်အခဲရှိနေပါသည်။ ခဏအကြာတွင် ထပ်မံကြိုးစားပါ။');
...
function renderMediaInput(){
  ...
  <div class="vcard"><div class="vtitle">🎧 အသံ / Video</div><p class="hint">Audio သို့မဟုတ် Video ဖိုင်ကို တင်ပါ။ 5MB အထိ အသုံးပြုနိုင်ပါသည်။</p>
    <div class="form-group"><label>Audio / Video File</label><input type="file" id="mediaFile" accept="audio/*,video/*,.mp3,.wav,.m4a,.aac,.ogg,.flac,.webm,.mp4,.mov,.mkv"></div>
  </div>
  <div class="vcard"><div class="vtitle">Output ရွေးချယ်ရန်</div><div class="choice-grid">
...
function submitMedia(type){
  readMedia(function(){
    ...
    }).catch(function(e){renderMediaError(e);});
  });
}
...
function startSrtMedia(){if(USER_PLAN!=='PRO'){toast('ဒီ Feature ကို Pro User သာ အသုံးပြုနိုင်ပါသည်','error');return;}renderSrtInput('media');}
...
function applyContentTransfer(){
  try{
    ...
    VOICE_STATE.source=d.source||'content';
    VOICE_STATE.voiceMode='text-to-voice';
    voiceStartMode('text-to-voice');
...
(function initVoice(){
  ...
  hydratePlan();restoreDraft();applyContentTransfer();
})();
</script>
</body></html>`;
